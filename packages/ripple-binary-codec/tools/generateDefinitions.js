;(async () => {
  ...
  const fetchFileFromGithub = async (repo, filename) => {
    if (!repo.includes('tree/')) repo += '/refs/heads/develop'
  ...
  const fs = require('fs')
  async function readFile(repo, filename) {
    if (repo.includes('github.com')) {
      try {
        return await fetchFileFromGithub(repo, filename)
      } catch (err) {
    ...
    try {
      return fs.readFileSync(repo + '/' + filename).toString('utf-8')
    } catch (err) {
      console.error(`Error reading file ${repo + '/' + filename}:`, err.message)
      process.exit(1)
    }
  }
  ...
  const sfieldMacroFile = await readFile(
    process.argv[2],
    '/include/xrpl/protocol/detail/sfields.macro',
  )
  const ledgerFormatsMacroFile = await readFile(
    process.argv[2],
    '/include/xrpl/protocol/detail/ledger_entries.macro',
  )
  const terFile = await readFile(
    process.argv[2],
    '/include/xrpl/protocol/TER.h',
  )
  const transactionsMacroFile = await readFile(
    process.argv[2],
