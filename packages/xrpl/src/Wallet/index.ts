...
export class Wallet {
   ...
   * @param opts.algorithm - The digital signature algorithm to generate an address for. If omitted,
   *                         the algorithm is inferred from the seed's base58 prefix: `sEd…` seeds
   *                         derive ed25519 keypairs; all other `s…` seeds derive secp256k1.
   * @param opts.masterAddress - Include if a Wallet uses a Regular Key Pair. It must be the master address of the account.
   ...
   * @param opts - (Optional) Options to derive a Wallet.
   * @param opts.algorithm - The digital signature algorithm to generate an address for. When undefined,
   *                         the algorithm is inferred from the seed's base58 prefix by
   *                         `ripple-keypairs.deriveKeypair` (via `decodeSeed`): `sEd…` → ed25519,
   *                         otherwise secp256k1. No fallback default is applied here, so an `s…`
   *                         secp256k1 seed will no longer silently derive an unrelated ed25519
   *                         account.
   * @param opts.masterAddress - Include if a Wallet uses a Regular Key Pair. It must be the master address of the account.
