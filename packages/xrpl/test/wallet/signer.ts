...
import { decode, encode, UpdatedXrplDefinitions } from 'ripple-binary-codec'
...
describe('Signer', function () {
  ...
  it('authorizeChannel succeeds with unrelated custom definitions', function () {
    const newDefs = new UpdatedXrplDefinitions(definitions)
    const secpWallet = Wallet.fromSeed('snGHNrPbHrdUcszeuDEigMdC1Lyyd')
    const channelId =
      '5DB01B7FFED6B67E6B0414DED11E051D2EE2B7619CE0EAA6286D67A3A4D5BDB3'
    const amount = '1000000'
  ...
  it('should be able to multisign a transaction with custom defs', async function () {
    ...
    const expectedMultisignWithNewDefs = { ...txToMultisign }
    expectedMultisignWithNewDefs.Signers = [
      {
        Signer: {
    ...
    expectedMultisignWithNewDefs.SigningPubKey = ''
