...
* `Wallet.fromSeed(seed)` and `Wallet.fromSecret(seed)` now infer the signing algorithm from the seed's base58 prefix when `opts.algorithm` is omitted: `sEd…` seeds derive ed25519 keypairs, and all other `s…` seeds derive secp256k1. Previously, both methods silently coerced an omitted algorithm to ed25519, which produced an unrelated ed25519 account for secp256k1 (`s…`) seeds. Callers that intentionally relied on the previous behavior to derive an ed25519 keypair from a non-`sEd` seed must now pass `{ algorithm: ECDSA.ed25519 }` explicitly. `Wallet.generate`, `Wallet.fromEntropy`, `Wallet.fromMnemonic`, and `walletFromSecretNumbers` are unaffected.
