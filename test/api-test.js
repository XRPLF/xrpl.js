...
describe('RippleAPI', function() {
  ...
  describe('Connection', function() {
    ...
    it('DisconnectedError on send', function() {
      this.api.connection._ws.send = function(message, unused, callback) {
    ...
    it('connection emit stream messages', function(done) {
      ...
      let pathFindCount = 0;
      this.api.connection.on('transaction', type => {
        transactionCount++;
      });
      this.api.connection.on('path_find', type => {
        pathFindCount++;
      });
      this.api.connection.on('1', type => {
  ...
  it('getPaths - source with issuer', function() {
    return this.api.getPaths(requests.getPaths.issuer).then(data => {
      assert(false, 'Should throw NotFoundError');
    }).catch(error => {
      assert(error instanceof this.api.errors.NotFoundError);
    });
