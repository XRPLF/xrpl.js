...
import { OfferLsfFlags } from '../../src/models/ledger/offer'
...
describe('client.getOrderbook', function () {
  ...
  it('direction is correct for buy and sell', async function () {
    ...
    assert.strictEqual(
      response.buy.every((item) => item.Flags !== OfferLsfFlags.lsfSell),
      true,
    )
    assert.strictEqual(
      response.sell.every((item) => item.Flags === OfferLsfFlags.lsfSell),
      true,
    )
  })
